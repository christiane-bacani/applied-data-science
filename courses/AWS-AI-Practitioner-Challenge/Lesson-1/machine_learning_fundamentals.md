...
There are two types of data used in *training* a model:
    - **Labeled data**: A dataset that each instance provides a label or desired variable or output provided by human experts through a reliable process (e.g, An animal image annotated with the class of that animal such as dog, cat, or wolf).
    - **Unlabeled data**: A dataset that each instance does not provide a label or desired variable or output.